...
### 프로젝트·마일스톤 연동 (선택, 1회 설정)

"이슈는 등록됐는데 프로젝트·마일스톤은 하나하나 수동 연동"하는 문제를 없애는 설정입니다.

- **프로젝트 (Projects v2)** — 팀 프로젝트 → ⚙️ Settings → **Workflows** → **Auto-add to project**를 켜고 필터를 `label:ai-task`로 지정하세요. 이후 `/spec`이 등록하는 모든 이슈가 (웹에서 수동 등록한 이슈도 라벨만 붙으면) **자동으로 프로젝트에 추가**됩니다. 커맨드 쪽 설정은 필요 없습니다.
- **마일스톤** — 저장소에 열린 마일스톤이 있으면 `/spec`이 이슈 등록 전에 **마감일이 지나지 않은 마일스톤을 마감 임박 순으로** 조회해 승인 단계에서 함께 확인받고, `gh issue create --milestone`으로 등록합니다. 마일스톤이 없으면 묻지 않습니다. 별도 설정은 필요 없고, 마일스톤에 마감일을 넣어두면 과거 마일스톤이 제안에 섞이지 않습니다.
...
3. 이슈 분해안을 보여주고 **승인 요청** ← 여기서 사람이 검토 (열린 마일스톤이 있으면 어디에 넣을지도 함께 확인)
