...
- **상단 고정 배너 의무**: `vN · 상태(승인 전/승인됨/구현됨) · 날짜 · "구현물 아님 — 합의용 시안"` — 브라우저로 여는 사람이 meta.json 없이도 상태를 알 수 있게
...
승인되면 meta.json의 `status`를 `approved`로, `approvedVersion`을 승인된 N으로 갱신한다. **승인 후 시안을 바꿔야 하면**: 이슈 등록 전이면 vN+1 + 재승인, 등록 후면 해당 이슈에 `needs-respec`을 붙이고 재설계 경로를 따른다.
